 ...
 * <code>ImmutableScalar</code> instances that were acquired from the same
 * <code>Session</code> and whose contents have not been read. The equality
 * comparison must not change the state of the <code>ImmutableScalar</code> instances even
 * though the <code>asString()</code> method in the above definition implies a
 * state change.
 */
@Nonnull
public interface ImmutableScalar extends ImmutableCoreScalar {
    @Nullable
    @Override
    Object getValue();
     ...
     *                               been called on this <code>ImmutableScalar</code> instance. In this case a new
     *                               <code>ImmutableScalar</code> instance must be acquired in order to successfully
     *                               call this method.
    ...
    default ImmutableBinaryScalar asBinaryValue() {
